...
class Database {
    ...
    public static function delete ($table, $where = '') {
        return DB::query('DELETE FROM ' . $table . ($where ? ' WHERE ' . (is_string($where) ? $where : self::parseWhere($where)) : ''));
    }

    public static function simpleSelect ($table, $fields, $where = '') {
        return DB::query('SELECT ' . $fields . ' FROM ' . $table . ($where ? ' WHERE ' . (is_string($where) ? $where : self::parseWhere($where)) : ''));
    }
