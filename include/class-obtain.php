...
class Obtain {
    ...
    */
    ...
    public static function TypeName($type, $typeb = 0, $image = FALSE, $appendclass = '') {
        ...
        $typearr    = array('', '火', '水', '草', '电', '普', '格', '飞', '虫', '毒', '岩', '地', '钢', '冰', '超', '恶', '鬼', '龙', '妖');
    ...
    public static function EggGroupName($group, $groupb = 0) {
        ...
        $grouparr    = array('', '陆上', '昆虫', '飞行', '怪兽', '妖精', '人形', '矿物', '植物', '水中1', '水中2', '水中3', '飞龙', '不定形', '百变怪', '未发现');
        ...
        switch($class) {
            case 'pokemon':
                ...
                if(count($data) < 5) {
                    ...
                    */
                    ...
                    $spot = array(
                        array($pv[7], $pv[6]), 
                        array($pv[5] + 24, $pv[4] + 2), 
                        array($pv[3] + 3, $pv[2] + 16), 
                        array($pv[1] + 15, $pv[0] + 18))
                    ;
            ...
            case 'other':
                ...
                */
                ...
                if(in_array($data[0], array('hp', 'exp'))) {
    ...
    */
    ...
    public static function Exp($exptype, $nextlevel){
        ...
        switch($exptype) {
            ...
            break;
            case 2:    $nextexp = 0.8 * pow($nextlevel, 3);                                                     break;
            case 3:    $nextexp = pow($nextlevel, 3);                                                           break;
            case 4:    $nextexp = 1.2 * pow($nextlevel, 3) - 15 * pow($nextlevel, 2) + 100 * $nextlevel - 140;  break;
            case 5:    $nextexp = 1.25 * pow($nextlevel, 3);                                                    break;
            case 6:
    ...
    public static function Stat($level, $bs, $iv, $ev, $nature = 1, $hp = FALSE) {
        ...
        $prefix = array('maxhp', 'atk', 'def', 'spatk', 'spdef', 'spd');
        foreach($prefix as $key => $val){
            switch($key){
                default:    $result[$val] = floor(floor(floor($bs[$key] * 2 + $ev[$key] / 4 + $iv[$key]) * $level / 100 + 5) * $modifier[$key]);                break;
                case 0:        $result['maxhp'] = ($bs[$key] != 1) ? floor(floor($bs[$key] * 2 + $ev[$key] / 4 + $iv[$key]) * $level / 100 + $level + 10) : 1;        break;
            }
        }
    ...
    public static function TrainerAvatar($uid, $size = 'middle') {
        ...
        $path    = 'uc_server/data/avatar/' . substr($uid, 0, 3) . '/' . 
            substr($uid, 3, 2) . '/' . 
            substr($uid, 5, 2) . '/' . 
            substr($uid, -2) . '_avatar_' . (in_array($size, array('big', 'middle', 'small')) ? $size : 'middle') . '.jpg';
