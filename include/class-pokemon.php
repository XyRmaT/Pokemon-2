...
class Pokemon {
    ...
    public static function Generate($id, $uid, $param = []){
        ...
        */
        ...
        $ivpart    = array(rand(0, 31), rand(0, 31), rand(0, 31), rand(0, 31), rand(0, 31), rand(0, 31));
        $iv        = implode(',', $ivpart);
        ...
        switch($pokemon['genderrt']) {
            case 255:   $gender = 0; break;
            case 254:   $gender = 2; break;
            case 0:     $gender = 1; break;
            default:    $gender = (('0x' . $pvpart[3]) * 1 >= $pokemon['genderrt']) ? 1 : 2; break;
        }
    ...
    */
    ...
    public static function Evolve(&$info, $param = array()) {
                ...
                */
                ...
                if($processtotal !== 0) {
                    ...
                    if(empty($param['useitem'])) {
                        ...
                        !empty($val[6]) && in_array($val[6], array($info['move'][0][0], $info['move'][1][0], $info['move'][2][0], $info['move'][3][0])) && ++$processcount;
                        ...
                        if($val[10]) {
                            ...
                            switch($val[10]) {
                                case 1: ($info['atk'] > $info['def']) && ++$processcount;                                    break;
                                case 2: ($info['atk'] < $info['def']) && ++$processcount;                                    break;
                                case 3: ($info['atk'] === $info['def']) && ++$processcount;                                    break;
                                case 4: ((('0x' . substr($info['pv'], 4, 4)) * 1) % 65535 % 10 < 5) && ++$processcount;        break;
                                case 5: ((('0x' . substr($info['pv'], 4, 4)) * 1) % 65535 % 10 >= 5) && ++$processcount;    break;
                            }
    ...
    */
    ...
    public static function Levelup(&$info, $rarecandy = 0) {
        ...
        */
                        ...
                        if(empty(self::$pmtmp['move'][$info['id']])) {
                            ...
                            while($infob = DB::fetch($query)) {
                                ...
                                if(Kit::ColumnSearch($info['move'], 0, $infob['mid']) === FALSE && Kit::ColumnSearch($info['newmove'], 0, $infob['mid']) === FALSE)
                                    ...
                                    self::$pmtmp['move'][$info['id']][$infob['level']][] = array($infob['mid'], $infob['name']);
        ...
        return array($diff['next'], $diff['now'], $remain['exp'], $remain['percent']); 
