...
require_once DISCUZ_ROOT . './../bbs/uc_client/lib/xml.class.php';
...
class uc_note {
    ...
    function _serialize($arr, $htmlon = 0) {
        if(!function_exists('xml_serialize')) {
            include_once DISCUZ_ROOT . './../bbs/uc_client/lib/xml.class.php';
        }
...
function _authcode($string, $operation = 'DECODE', $key = '', $expiry = 0) {
    ...
    $keyc =
        $ckey_length ? ($operation == 'DECODE' ? substr($string, 0, $ckey_length) : substr(md5(microtime()), -$ckey_length)) : '';

    $cryptkey   = $keya . md5($keya . $keyc);
    $key_length = strlen($cryptkey);

    $string        =
        $operation == 'DECODE' ? base64_decode(substr($string, $ckey_length)) : sprintf('%010d', $expiry ? $expiry + time() : 0) . substr(md5($string . $keyb), 0, 16) . $string;
