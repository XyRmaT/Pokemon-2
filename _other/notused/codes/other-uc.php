...
if(!defined('UC_API')) exit('Access denied');

define('IN_UC', TRUE);
define('UC_CLIENT_VERSION', '1.6.0');
define('UC_CLIENT_RELEASE', '20110501');
define('UC_ROOT', ROOT . '/../bbs/uc_client/');
...
function uc_user_register($username, $password, $email, $questionid = '', $answer = '', $regip = '') {
    return call_user_func(UC_API_FUNC, 'user', 'register', ['username' => $username, 'password' => $password, 'email' => $email, 'questionid' => $questionid, 'answer' => $answer, 'regip' => $regip]);
}

function uc_user_login($username, $password, $isuid = 0, $checkques = 0, $questionid = '', $answer = '') {
    $isuid  = intval($isuid);
    $return = call_user_func(UC_API_FUNC, 'user', 'login', ['username' => $username, 'password' => $password, 'isuid' => $isuid, 'checkques' => $checkques, 'questionid' => $questionid, 'answer' => $answer]);
    return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
...
function uc_user_synlogin($uid) {
    $uid = intval($uid);
    $return = '';
    if(@(include UC_ROOT . './data/cache/apps.php') && !empty($_CACHE) && count($_CACHE['apps']) > 1)
...
function uc_user_edit($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '') {
    return call_user_func(UC_API_FUNC, 'user', 'edit', ['username' => $username, 'oldpw' => $oldpw, 'newpw' => $newpw, 'email' => $email, 'ignoreoldpw' => $ignoreoldpw, 'questionid' => $questionid, 'answer' => $answer]);
}

function uc_user_delete($uid) {
    return call_user_func(UC_API_FUNC, 'user', 'delete', ['uid' => $uid]);
}
