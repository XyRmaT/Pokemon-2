	...
	default:
		...
		*/
		...
		while($info = DB::fetch($query)) {
			...
			$info['hltime']		= array(floor($info['rmtime'] / 60 / 24), round($info['rmtime'] / 60));
		...
		if($blank < 3) {
			$heal = array_merge($heal, array_fill($count, $blank, array('pkmimgpath' => Obtain::Sprite('pokemon', 'png', '0'))));
		}
		...
		*/
		...
		$blank = abs(($count - 6) + (floor($count / 3) ^ 1) * 3);
		if($blank < 3) {
			$pokemon = array_merge($pokemon, array_fill($count, $blank, array('pkmimgpath' => Obtain::Sprite('pokemon', 'png', '0'))));
		}
	break;
	case 'box':
