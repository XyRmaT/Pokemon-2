...
switch($_GET['section']) {
	case '':
		...
		*/
		...
		while($info = DB::fetch($query)) {
			switch($info['nat_id']) {
				...
				default:
					...
					foreach($info['moves'] as $val) $movecriteria[] = $val[0];
		...
		if($movecriteria) {
			$query = DB::query('SELECT move_id, type, power, class FROM pkm_movedata WHERE move_id IN (' . implode(',', $movecriteria) . ')');
			$move  = [];
			while($info = DB::fetch($query))
				$move[$info['move_id']] = [$info['type'], $info['power'], Obtain::MoveClassName($info['class']), Obtain::TypeName($info['type'])];
		}
