...
$item    = [];
$query   = DB::query('SELECT iid, name, dscptn, price, store, trnrlv, mthsell FROM pkm_itemdata WHERE sell = 1 AND type = ' . $type . ' AND trnrlv <= ' . $user['level'] . ' AND (timestt = 0 AND timefns = 0 OR timestt < ' . $_SERVER['REQUEST_TIME'] . ' AND timefns > ' . $_SERVER['REQUEST_TIME'] . ') ORDER BY price ASC LIMIT ' . $start . ', ' . $limit);
