...
$query   = DB::query('SELECT m.pkm_id, m.nat_id, m.ind_value, m.gender, m.nature, m.moves, m.level, m.exp, m.location, m.uid,
                              p.name_zh name, a.name_zh ability, mb.username
                      FROM pkm_mypkm m
                      LEFT JOIN pkm_pkmdata p ON p.nat_id = m.nat_id AND m.nat_id != 0
                      LEFT JOIN pkm_abilitydata a ON a.abi_id = m.ability
                      LEFT JOIN pre_common_member mb ON mb.uid = m.uid
                      ORDER BY m.pkm_id ASC');
$pokemon = [];
