...
switch($_GET['process']) {
	...
	case 'pcheal':
		...
		if($_GET['asd'] == 1) {print_r('<pre>');print_r($pokemon);}
	...
	case 'boxmove':
		...
		$place	= $pokemon = $curplace = $unable = $sql = array();
		...
		foreach($unable as $key => $val) {
			...
			switch($key) {
				case 1:		$return['msg'] .= '身上的精灵太多了，' . $val . '只精灵移动失败！<br>';						break;
				default:	$return['msg'] .= ($key - 100) . '号箱子的精灵太多了，' . $val . '只精灵移动失败！<br>';	break;
			}
		...
		if(!empty($sql)) {
			...
			Kit::Library('class', array('pokemon'));
	...
	case 'tradeaccept':
		...
		*/
		...
		foreach($pokemon as $key => $val) {
			...
			Pokemon::Evolve($pokemon[$key], array('other' => !0, 'otherobj' => $pokemon[$key ^ 1]['id'], 'uid' => $pokemon[$key ^ 1]['uid']));
