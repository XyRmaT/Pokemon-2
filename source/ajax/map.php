...
switch($_GET['process']) {
    case 'update':
        ...
        $_GET['mapid']  = 1;
        $query          = DB::query('SELECT uid, username, x, y, mpid FROM pkm_mapcoordinate WHERE mpid = ' . intval($_GET['mapid']) . ' AND uid != ' . $_G['uid']);
        $data           = [];
    ...
    case 'walk':
        ...
        $mapid = DB::result_first('SELECT mpid FROM pkm_mapcoordinate WHERE uid = ' . $_G['uid']);
        ...
        DB::query('INSERT INTO pkm_mapcoordinate (uid, username, x, y, mpid, time)
                   VALUES (' . $_G['uid'] . ', \'' . $_G['username'] . '\', ' . $x . ', ' . $y . ', ' . $mapid . ', ' . $_SERVER['REQUEST_TIME'] . ')
                   ON DUPLICATE KEY UPDATE x = VALUES(x), y = VALUES(y), time = VALUES(time)');
        
        $return['console'] = '';
        
        if(DB::result_first('SELECT COUNT(*) FROM pkm_mypkm WHERE place IN (1, 2, 3, 4, 5, 6) AND hp > 0 AND id != 0 AND uid = ' . $user['uid']) > 0) {
            ...
            while($info = DB::fetch($query)) {
                ...
                $info['newmove']        = !empty($info['newmove']) ? unserialize($info['newmove']) : array();
            ...
            $tmp    = array();
            $query    = DB::query('SELECT id, levelmin, levelmax, rate FROM pkm_encounterdata WHERE mpid = ' . $mapid . ' AND timefrom < ' . $_SERVER['REQUEST_TIME'] . ' AND (timeto = 0 OR timeto > ' . $_SERVER['REQUEST_TIME'] . ') AND qty != 0');
            ...
            DB::query('UPDATE pkm_trainerdata SET inbtl = 1 WHERE uid = ' . $user['uid']);

            Battle::$pokemon[0] = array(
                Pokemon::Generate($appearpkm['id'], $user['uid'], array(
                    'mtplace'    => $mapid, 
            ...
            Battle::WriteBattleData($_G['uid'], Battle::$pokemon);
            
            Battle::$report = '野生的' . Battle::$pokemon[0][0]['name'] . '出现了！<br>' . $_G['username'] . '派出了' . Battle::$pokemon[1][0]['name'] . '！<br>';
            ...
            if(MODE === 1) {
                ...
                while($info = DB::fetch($query)) {
                    ...
                    while($infob = DB::fetch($queryb)) {
                        $move[] = array($infob['mid'], $infob['pp'], $infob['name'], $infob['pp'], 0);
                    }
                ...
                while($info = DB::fetch($query)) {
                    ...
                    foreach($info['move'] as $key => $val) {
                        $info['move'][$key] = array($val[0], $move[$val[0]]['pp'], $move[$val[0]]['name'], $move[$val[0]]['pp'], 0);
                    }
