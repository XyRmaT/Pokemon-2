...
<table class="shop-list">
	...
	<!--{loop $item $val}-->
		<tr id="i$val[item_id]" class="item"><td><div style="background-image:url($val[carry_item_sprite])"></div></td><td>$val[name]</td><td>$val[description]</td><td>$val[price]</td><td>$val[stock]</td><td>$val[month_sale] ($val[mthsellper]%)</td><td><button data-itemid="$val[item_id]" data-name="$val[name]">购买</button></td></tr>
	<!--{/loop}-->
</table>
